...
cap = cv2.VideoCapture('sanJac4.avi')
...
color = (0,255, 30)
1
cap.set(cv2.cv.CV_CAP_PROP_POS_FRAMES,22) #Tell the video capture which frame I want to grab, when I read from it in the next line.
...
for s in range(0,88):
    ...
    good_old = p0[st==1]
    dist = []
   # cv2.circle(frame,(800,520),15,[0,255,255],-1,8) 
    center = rows/2
    maxDist = 0
    h = 20
    w = 40
...
#        if distTemp > 0.3 and distTemp < 1 and a > 580 and b > 520 and b < 900 and a< 750:
        if distTemp > 2 and distTemp < 5 and a > 500 and a < 810 and b > 440 and b < 520 and n>2:
            #
