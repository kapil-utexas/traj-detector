...
cap = cv2.VideoCapture('sanJac3.avi')

# params for ShiTomasi corner detection
feature_params = dict( maxCorners = 550,
                       qualityLevel = 0.21,
                       minDistance = 3.5,
                       blockSize = 3  )
...
cap.set(cv2.cv.CV_CAP_PROP_POS_FRAMES,230) #Tell the video capture which frame I want to grab, when I read from it in the next line.
...
for s in range(0,100):
    ...
    for i,(new,old) in enumerate(zip(good_new,good_old)):
        ...
        if distTemp > 0.3 and distTemp < 3 and a > 580 and b > 520 and b < 900 and a< 750:
            #
