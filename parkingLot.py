...
cap.set(cv2.cv.CV_CAP_PROP_POS_FRAMES,190) #Tell the video capture which frame I want to grab, when I read from it in the next line.
# Take first frame and find corners in it
ret, old_frame = cap.read()
old_frame = cv2.imread('mask88.jpg')
...
#while(1):
n = 88
for s in range(88,240):
    n =n + 1
    ret,frame = cap.read()
    frame = cv2.imread(''.join(['mask',str(n),'.jpg']))
    orig = cv2.imread(''.join(['orig',str(n),'.jpg']))
    ...
    for i,(new,old) in enumerate(zip(good_new,good_old)):
        ...
        cv2.circle(orig,(a,b),5,[0,0,255],-1,8)
        x_new = a - cols/2
        y_new = b - rows/2
        x_old = c - cols/2
        y_old = d - rows/2
    ...
    #cv2.rectangle(orig,(int(xKeep)-w/2,int(yKeep)-h/2),(int(xKeep)+w/2,int(yKeep)+h/2),[0,255,0],2)
    img = cv2.add(orig,mask)
